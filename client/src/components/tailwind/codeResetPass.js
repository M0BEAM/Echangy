...
function CRP() {
    ...
    const { email } = useContext(ResetPassCtx)
    ...
    const handeleResendCode = async () =>{
        try {
            ...
            await axios.get(process.env.REACT_APP_APIURL+"resendMailCodeReset",{
                headers:{
    ...
    const handeleSubmit =  async (e) => {
       ...
       if(code === ""){
        ...
        try {
            ...
            await axios.get(process.env.REACT_APP_APIURL + "codeResetPass/" + code, {
                headers: {
                    "X-API-Key": process.env.REACT_APP_APIKEY
                }
            })
            localStorage.setItem("Code",true)
            navigate("/resetPassword/updatePassword/"+localStorage.getItem("tokenResetPass"))
        } catch (error) {
