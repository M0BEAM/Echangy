...
export default function ConfModal({options}) {
    const { language } = useContext(LanguageCtx)
    const { openConf,setOpenConf } = options
    const translations = require(`../lang/${language}.json`).viewProfile;
   
    const cancelButtonRef = useRef(null)

    return (
        <Transition.Root show={openConf} as={Fragment}>
            <Dialog as="div" className="relative  z-10" initialFocus={cancelButtonRef} onClose={setOpenConf}>
                <Transition.Child
                    as={Fragment}
                            ...
                            <Dialog.Panel className="relative transform w-full overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                                <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                                    <div className="flex sm:items-start justify-center ">
                                        ...
                                        <div className="text- w-full sm:ml-4  sm:text-left">

                                        <h3 className='text-blue-900'>
                                        Code has been send !
                                                
                                        </h3>
                                        <h5 className='text-blue-900'>Check Your Mail ...</h5>
                                        </div>
                                    </div>
                                </div>
                            </Dialog.Panel>
